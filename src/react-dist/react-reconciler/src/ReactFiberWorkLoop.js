...
import { resetAfterCommit, scheduleTimeout, cancelTimeout, noTimeout, afterActiveInstanceBlur, getCurrentEventPriority, supportsMicrotasks, errorHydratingContainer, scheduleMicrotask, prepareRendererToRender, resetRendererAfterRender } from "../../react-dom-bindings/src/client/ReactDOMHostConfig";
...
import { getSuspenseHandler, getShellBoundary } from './ReactFiberSuspenseContext';
...
export var NoContext = /*             */
0;
var BatchedContext = /*               */
1;
export var RenderContext = /*         */
2;
export var CommitContext = /*         */
4;
...
function ensureRootIsScheduled(root, currentTime) {
  ...
  // on the `act` queue.
  !(false && ReactCurrentActQueue.current !== null && existingCallbackNode !== fakeActCallbackNode)) {
...
export function isAlreadyRendering() {
  ...
  // the wrong context.
  return false && (executionContext & (RenderContext | CommitContext)) !== NoContext;
}
...
function shouldAttemptToSuspendUntilDataResolves() {
  ...
  if (handler === null) {// TODO: We should support suspending in the case where there's no
...
function replaySuspendedUnitOfWork(unitOfWork) {
  ...
  var next;
  setCurrentDebugFiberInDEV(unitOfWork);
  var isProfilingMode = enableProfilerTimer && (unitOfWork.mode & ProfileMode) !== NoMode;
  if (isProfilingMode) {
    startProfilerTimer(unitOfWork);
  }
